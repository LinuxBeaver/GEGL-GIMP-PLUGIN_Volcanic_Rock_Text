...
switch (o->rocktype) {
  ...
  gegl_node_connect_from (state->hardlight, "aux", state->cellnoise, "output");
  gegl_node_link_many (state->idref, state->median2, state->gaussian, state->opacity2, NULL);
  gegl_node_connect_from (state->screen, "aux", state->opacity2, "output");
        break;
    case volcanicnolava:
  ...
  gegl_node_link_many (state->idref, state->median2, state->gaussian, state->opacity2, NULL);
  gegl_node_connect_from (state->screen, "aux", state->opacity2, "output");
  gegl_node_connect_from (state->replace, "aux", state->string2, "output");
  gegl_node_connect_from (state->replace2, "aux", state->lavarealtime, "output");
...
GeglOperationMetaClass *operation_meta_class = GEGL_OPERATION_META_CLASS (klass);
  ...
  gegl_operation_class_set_keys (operation_class,
    ...
    "title",       _("Chipped Rock, Volcanic Rock, and Lava Rock"),
